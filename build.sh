...
function main() {
    ...
    # docker buildx build --platform linux/arm64 -f Dockerfile.c -t c:latest --cache-from tmux:latest . || exit 1
    # docker build . -f Dockerfile.java -t java:latest --rm --no-cache || exit 1
    # docker buildx build --platform linux/amd64 -f Dockerfile.vim -t vim:latest --rm --no-cache . || exit 1
    docker buildx build --progress=plain --platform ${arch} -f Dockerfile.nvim --no-cache -t nvim:latest . || exit 1
    docker buildx build --progress=plain --platform ${arch} -f Dockerfile.fish -t fish:latest . || exit 1
    docker buildx build --progress=plain --platform ${arch} -f Dockerfile.my -t my:${today} . || exit 1
