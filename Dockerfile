...
RUN wget https://obs-community.obs.cn-north-1.myhuaweicloud.com/obsutil/current/obsutil_linux_amd64.tar.gz -P /opt \
    ...
    && cp /opt/obsutil_linux_amd64_5.2.5/obsutil /usr/bin \
...
RUN obsutil cp obs://lijian-test/Python-3.8.5.tar.xz /opt \
    && tar xJvf /opt/Python-3.8.5.tar.xz -C /opt \
    && cd /opt/Python-3.8.5 \
...
RUN cd /opt \
    ...
    && obsutil cp obs://lijian-test/vim-8.2.1359.tar.gz . \
    && tar xzvf vim-8.2.1359.tar.gz \
    && cd /opt/vim-8.2.1359 \
